...
public:
    ...
    virtual void Load(int count, string line) = 0; // дл€ выгрузки данных из файла
    virtual string Save(int count) = 0; // дл€ выгрузки данных в файл
    virtual int TypeID() = 0; // чтобы пон€т студент или  учитель или администраци€
};
...
public:
    ...
    Book(const Book& B); // конструктор копировани€
    ~Book(); // деструктор
...
private:
    ...
    string year_of_study; // год обучени€
    string volume_of_page; // объем страниц
    string cost; // цена
...
public:
    ...
    StudyBook(const StudyBook& S); // конструктор копировани€
...
public:
    ...
    Chancellery(const Chancellery& C); // конструктор копировани€
    ~Chancellery(); // деструктор

    // создание основных методов
    void Set() override; // добавление данных о канцел€рии
    void Get() override; // получение данных о канцел€рии
    void Delete() override; // удаление данных о канцел€рии
    void Change() override; // изменение данных о канцел€рии
    void Load(int count, string line) override;
    string Save(int count) override;
    int TypeID() override;
};
